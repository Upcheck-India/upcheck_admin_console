...
import { CheckCircle, XCircle, Mail, Building2, Calendar, Shield, Trash2, AlertTriangle } from 'lucide-react';

export default function ExternalUsersTab({ users, onApprove, onReject, onDelete, loading }) {
                  ...
                  {user.status === 'pending_approval' && (
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        title="Delete user from database and Clerk"
                      >
                  ...
                  {user.status === 'active' && (
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => handleDeleteClick(user._id)}
                        disabled={processingId === user._id}
                        className="text-red-600 hover:text-red-900 disabled:opacity-50 flex items-center ml-2"
                        title="Delete user from database and Clerk"
                      >
                        <Trash2 className="h-5 w-5" />
                      </button>
                  ...
                  {user.status === 'rejected' && (
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => handleDeleteClick(user._id)}
                        disabled={processingId === user._id}
                        className="text-gray-600 hover:text-gray-900 disabled:opacity-50 flex items-center ml-2"
                        title="Delete user from database and Clerk"
                      >
                        <Trash2 className="h-5 w-5" />
                      </button>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                  <label className="flex items-center">
                    ...
                    <span className="ml-2 text-sm text-gray-700">
                      Also delete from Clerk (removes user's ability to login)
                    </span>
                  </label>
                </div>
              </div>
