...
const EditSurvey = ({ params }) => {
  ...
  useEffect(() => {
    checkAuthAndFetchSurvey();
  }, []);

  const checkAuthAndFetchSurvey = async () => {
    try {
    ...
    try {
      ...
      if (surveyData.settings) {
        if (surveyData.settings.startDate) {
          surveyData.settings.startDate = new Date(surveyData.settings.startDate).toISOString().split('T')[0];
        }
        if (surveyData.settings.endDate) {
          surveyData.settings.endDate = new Date(surveyData.settings.endDate).toISOString().split('T')[0];
        }
      }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <h1 className="text-2xl font-bold flex items-center">
          <Edit2 className="mr-2" /> Edit Survey
        </h1>
      </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 capitalize">{formData.status}</p>
                  </div>
                </div>
                <div className="mt-2">
