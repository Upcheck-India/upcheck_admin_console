...
import { X, Trash2, Folder, FileText, AlertTriangle, Archive, Move, Users, Shield } from 'lucide-react';

export default function ProjectDeleteConfirmModal({ project, onClose, onConfirm, onMassMove, onArchive }) {
  ...
  const fetchPreview = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/projects/${project._id}/preview-delete`);
      if (response.ok) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-red-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-gray-900">Delete Project Space</h3>
            ...
            <>
              ...
              <div className="flex items-center gap-3 p-4 bg-red-50 rounded-xl border border-red-100 mb-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-red-900 truncate">{preview.project.name}</p>
                  <p className="text-sm text-red-600">
                    Status: <span className="capitalize">{preview.project.status}</span>
                    {preview.project.superManager && ` • Manager: ${preview.project.superManager}`}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3 mb-4">
                ...
                <div className="bg-gray-50 rounded-xl p-3 text-center">
                  <p className="text-2xl font-bold text-gray-900">{preview.stats.memberCount}</p>
              ...
              {/* Warning and Archive suggestion */}
              <div className="mt-4 space-y-3">
                ...
                <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                  <p className="text-sm text-blue-800 flex items-start gap-2">
                    ...
                    <strong>Consider archiving instead:</strong> Archiving preserves all data while hiding the project from active view. You can restore it later if needed.
                  </p>
                </div>
              </div>
            </>
